...
RUN sed -i 's/idp.example.org/idp.nnmp.leanon.se/g' /opt/shibboleth-identityprovider-2.4.3/src/installer/resources/install.properties
...
ADD spring-sample-sp.xml /opt/shibboleth-idp/metadata/urntestjladenforsuppsala_sp.xml
ADD handler.xml /opt/shibboleth-idp/conf/
ADD login.config /opt/shibboleth-idp/conf/
ADD relying-party.xml /opt/shibboleth-idp/conf/
ADD attribute-filter.xml /opt/shibboleth-idp/conf/
ADD attribute-resolver.xml /opt/shibboleth-idp/conf/

ADD jaas-sample-1.0-SNAPSHOT.jar /usr/share/tomcat6/lib/
...
RUN chown -R tomcat /opt/shibboleth-idp/credentials
RUN chown -R tomcat /opt/shibboleth-idp/war

RUN rm -rf /opt/shibboleth-identityprovider-*
